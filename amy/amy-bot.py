...
def main():
    ...
    def update_fair_value(message):
        ...
        if message["buy"] and fair_value[symbol] and message["buy"][0][0] > fair_value[symbol]:
            exchange.send_add_message(symbol=symbol, dir=Dir.SELL, price=message["buy"][0][0], size=50)
        if message["sell"] and fair_value[symbol] and message["sell"][0][0] < fair_value[symbol]:
